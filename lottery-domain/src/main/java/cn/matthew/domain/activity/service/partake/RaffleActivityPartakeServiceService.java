...
import cn.matthew.types.utils.SnowflakeDistributeId;
...
@Service
@Slf4j
public class RaffleActivityPartakeServiceService extends AbstractRaffleActivityPartakeService {

    SnowflakeDistributeId idWorker = new SnowflakeDistributeId(0, 0);
    private final SimpleDateFormat dateFormatMonth = new SimpleDateFormat("yyyy-MM");
    private final SimpleDateFormat dateFormatDay = new SimpleDateFormat("yyyy-MM-dd");

    public RaffleActivityPartakeServiceService(IActivityRepository activityRepository) {
...
//        userRaffleOrder.setOrderId(String.valueOf(UUID.randomUUID().toString().replace("-", "")));
        userRaffleOrder.setOrderId(String.valueOf(idWorker.nextId()));
        log.info("orderId是：{}", userRaffleOrder.getOrderId());
